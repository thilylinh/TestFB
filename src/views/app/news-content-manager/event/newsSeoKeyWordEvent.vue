    ...
    <b-form class="content-form ndev-form">
      <b-row>
        <b-col lg="12" md="12" sm="12" xs="12">
          <label class="font-weight-700">1. Tên từ khóa SEO</label>
        </b-col>
        <b-col lg="12" md="12" sm="12" xs="12">
          <b-form-input id="roomManagerEvent_txtTen"
                        v-model="$v.form.name.$model"
                        :state="!$v.form.name.$error"
                        autocomplete="off"
                        autofocus
                        placeholder="Nhập tên từ khóa SEO ..."/>
      ...
      <n-core-button-run :btnSaveTitle="'Lưu dữ liệu'"
                         :showBtnSave="'1'"
                         ref="refnewsSeoKeyWordBtnSave"
                         @submitEvent="subEvnewsSeoKeyWordBtnSave"/>
    </b-form>
  </b-modal>
</template>
...
export default {
  ...
  data() {
    return {
      idModal: "newsSeoKeyWordEventModalId",
      titleModal: "",
      modalData: [],
      form: {
        description: "",
  ...
  methods: {
    ...
    async subEvnewsSeoKeyWordBtnSave(isSave) {
      if (isSave === true) {
        this.$v.$touch();
        this.$v.form.$touch();
          ...
          const iResult =
            this.modalData.type === "add"
              ? await NCoreHelper.v2executePOST(
                this,
                newsSeoKeyWordApi.NEWS_SEO_KEYWORD_CREATE,
                this.form
              )
              : await NCoreHelper.v2executePUT(
                this,
                newsSeoKeyWordApi.NEWS_SEO_KEYWORD_UPDATE,
                this.form
              );
          if (iResult) {
            ...
            this.$refs.refnewsSeoKeyWordBtnSave.processing = false
          } else {
            NCoreHelper.v2alertMess(
              this,
              NCoreHelper.returnMessage(
