<template>
  <b-row>
    <b-colxx class="disable-text-selection">
      <b-row>
        <b-colxx xxs="12">
          <h1 class="NHeaderPage">Quản lý Via</h1>
          ...
          <b-colxx xxs="12" class="NTable">
            <vuetable ref="refNewsGroupTypeTable"
              ...
              <template slot="roomName" slot-scope="props">
                <span class="NTitleDataInTable">{{ props.rowData.code }}</span>
                <small style="font-style: italic;color: red;"
                ><br/>{{ props.rowData.idTkQc }}</small>
              ...
              <template slot="staffName" slot-scope="props">{{props.rowData.staffName}}</template>
    ...
    <news-via-event ref="refNewsGroupTypeEvent"
...
import NewsViaEvent from "@/views/app/news-content-manager/event/newsViaEvent.vue";
  ...
  methods: {
    ...
    async fetchData() {
      ...
      const iResult = await NCoreHelper.v2executeGET(
        this,
        newsViaApi.NEWS_VIA_GET_PAGING,
        dataSend
      );
    ...
    async subEvNewsGroupTypeButtonDefaultAddNew() {
      const valueAuthority = await NCoreHelper.v2checkAuthorities('NewsVia')
