<template>
  <b-row>
    <b-colxx class="disable-text-selection">
      <b-row>
        <b-colxx xxs="12">
          <h1 class="NHeaderPage">Quản lý từ khóa SEO</h1>
          <div class="top-right-button-container">
            <n-button-default-add-new ref="refNewsSeoKeyWordButtonDefaultAddNew"
                                      @handlerSubmitEvent="subEvNewsSeoKeyWordButtonDefaultAddNew"/>
          </div>
          <piaf-breadcrumb/>
          <div class="mb-2">
            ...
            <b-collapse id="displayOptions" class="d-md-block">
              <div class="d-block d-md-inline-block pt-1">
                ...
                <n-core-statu-combobox-view @handlerSubmitEvent="subEvNewsSeoKeyWordStatusViewCombobox"/>
              </div>
      ...
      <template>
        ...
        <b-row v-show="!$store.state.componentLoading">
          <b-colxx xxs="12" class="NTable">
            <vuetable ref="refNewsSeoKeyWordTable"
                      :api-mode="false"
                      :data="dataPage"
                      :fields="fields"
              ...
              <template slot="actions" slot-scope="props">
                <n-core-action :data-id="props.rowData.id"
                               ...
                               @handlerSubmitEvent="subEvNewsSeoKeyWordAction"/>
              </template>
            </vuetable>
    ...
    <news-seo-key-word-event ref="refNewsSeoKeyWordEvent"
                             @handleSubmitEvent="subEvNewsSeoKeyWordEvent"/>
    <n-core-modal-delete ref="refNewsSeoKeyWordDelete"
                         @handleSubmitEvent="subEvNewsSeoKeyWordDelete"/>
    <n-core-modal-question ref="refNewsSeoKeyWordQuestion"
                           @handleSubmitEvent="subEvNewsSeoKeyWordQuestion"/>
  </b-row>
</template>
<script>
...
import NewsSeoKeyWordEvent from "@/views/app/news-content-manager/event/newsSeoKeyWordEvent";
import newsSeoKeyWordApi from "@/views/app/news-content-manager/api/newsSeoKeyWordApi";
  ...
  methods: {
    ...
    async fetchData() {
      ...
      const iResult = await NCoreHelper.v2executeGET(
        this,
        newsSeoKeyWordApi.NEWS_SEO_KEYWORD_GET_PAGING,
        dataSend
      );
    ...
    async subEvNewsSeoKeyWordStatusViewCombobox(val) {
      this.statusId = val
      this.pageNumber = 1
      await this.fetchData()
    },
    async subEvNewsSeoKeyWordButtonDefaultAddNew() {
      const valueAuthority = await NCoreHelper.v2checkAuthorities('NewsSeoKeyWord')
      if ((valueAuthority & NCoreConfig.authority_Add) !== 0) {
        await this.callStoreCrudUpdate({
          id: this.$refs.refNewsSeoKeyWordEvent.idModal,
          title: "Thêm mới từ khóa SEO",
          type: "add",
          funcName: "từ khóa SEO",
          data: null
        });
        NCoreHelper.v2modalShowHide(
          this,
          this.$refs.refNewsSeoKeyWordEvent.idModal,
          1
        );
      } else {
    ...
    async subEvNewsSeoKeyWordEvent(val) {
      if (val === true) {
        await this.fetchData();
      }
      NCoreHelper.v2modalShowHide(
    ...
    async subEvNewsSeoKeyWordDelete(val) {
      let model = [];
      model.push(val.id);
      const iResult = await NCoreHelper.v2executePOST(
        this,
        newsSeoKeyWordApi.NEWS_SEO_KEYWORD_DELETE,
        {
          models: model
        }
      );
      if (iResult) {
        ...
        NCoreHelper.v2modalShowHide(
          this,
          this.$refs.refNewsSeoKeyWordDelete.idModal,
          0
        );
      } else {
    ...
    async subEvNewsSeoKeyWordQuestion(val) {
      if (val.result) {
        await this.fetchData()
        NCoreHelper.v2modalShowHide(
          this,
          this.$refs.refNewsSeoKeyWordQuestion.idModal,
          0
        );
    ...
    async subEvNewsSeoKeyWordAction(val) {
      switch (val.id) {
        // Mở khóa
        case 201: {
          await this.callStoreCrudQuestion({
            id: this.$refs.refNewsSeoKeyWordQuestion.idModal,
            tableId: 5,
        ...
        case 101: {
          ...
          await this.callStoreCrudUpdate({
            id: this.$refs.refNewsSeoKeyWordEvent.idModal,
            title: "Cập nhật từ khóa SEO",
            type: "update",
            funcName: "từ khóa SEO",
            data: await NCoreHelper.v2executeGETById(
              this,
              newsSeoKeyWordApi.NEWS_SEO_KEYWORD_GET_BY_ID,
              val.dataContentId
            )
          });
          NCoreHelper.v2modalShowHide(
