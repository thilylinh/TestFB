<template>
  <b-modal :id="pageId" :hide-footer="true" :hide-header-close="false" :title="pageTitle" cancel-title="Hủy bỏ"
    cancel-variant="outline-primary" centered no-close-on-backdrop ok-title="Lưu" size="md" @shown="callOpenPage"
      ...
      <b-row>
        <b-colxx xxs="12">
          <b-row style="display: none;">
            ...
            <b-colxx xxs="12">
              <b-form-input id="uploadEvent_txtName" v-model="$v.form.name.$model" :state="!$v.form.name.$error"
                autocomplete="off" autofocus placeholder="Nhập tên hình ảnh ..." />
          ...
          <b-row class="mt-2">
            <b-colxx xxs="12">
              <label class="font-weight-700">*. Đường dẫn <span class="nRequired">(*)</span> </label>
            </b-colxx>
            <b-colxx xxs="12">
              <b-form-input id="uploadEvent_txtName" v-model="$v.form.link.$model" :state="!$v.form.link.$error"
                autocomplete="off" placeholder="Nhập đường dẫn hình ảnh ..." />
              <b-form-invalid-feedback v-if="!$v.form.link.required">
                Không được bỏ trống
              </b-form-invalid-feedback>
            </b-colxx>
            <b-colxx xxs="12" class="center">
              <b-img-lazy style="width: 150px;height: auto" :src="form.link" />
            </b-colxx>
          </b-row>
        </b-colxx>
      </b-row>

      <n-core-button-run :btnCancel="'Quay về'" :btnSaveTitle="'Lưu'" :showBtnSave="'1'"
...
import libraryManagerApi from "@/views/app/news-content-manager/api/libraryManagerApi";
...
export default {
  ...
  data() {
    return {
      pageId: "uploadImageFromDifferenceLinkPageId",
  ...
  methods: {
    ...
    resetStateModal() {
      //this.form.name = moment().format('ddMMyyyyHHss')
      this.form.link = '';
    },
