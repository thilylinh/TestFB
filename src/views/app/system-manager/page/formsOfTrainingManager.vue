<template>
  <b-row>
    <b-colxx class="disable-text-selection">
      <b-row>
        <b-colxx xxs="12">
          <h1 class="NHeaderPage">Quản lý Hình thức đào tạo</h1>
...
export default {
  ...
  data() {
    return {
      pageId: "formsOfTrainingManagerPageId",
  ...
  methods: {
    ...
    async fetchData() {
      ...
      const iResult = await NCoreHelper.v2executeGET(
        this,
        formsOfTrainingManagerApi.GET_PAGING,
        dataSend
      );
    ...
    async subEvSubjectTypeManagerButtonDefaultAddNew() {
      const valueAuthority = await NCoreHelper.v2checkAuthorities('FormsOfTrainingManager')
    ...
    async subEvSubjectTypeManagerDelete(val) {
      ...
      const iResult = await NCoreHelper.v2executePOST(
        this,
        formsOfTrainingManagerApi.DELETE,
        {
          models: model
        }
      );
    ...
    async subEvSubjectTypeManagerAction(val) {
      switch (val.id) {
        ...
        case 101: {
          ...
          await this.callStoreCrudUpdate({
            id: this.$refs.refSubjectTypeManagerEvent.idModal,
            title: "Cập nhật hình thức đào tạo",
            type: "update",
            funcName: "hình thức đào tạo",
            groupTable: 17,
