<template>
  <b-row>
    <b-colxx class="disable-text-selection">
      <b-row>
        <b-colxx xxs="12">
          <h1 class="NHeaderPage">Quản lý kiểu người dùng</h1>
  ...
  methods: {
    ...
    async fetchData() {
      ...
      const iResult = await NCoreHelper.v2executeGET(
        this,
        userTypeManagerApi.USER_TYPE_MANAGER_GET_PAGING,
        dataSend
      );
    ...
    async subEvSubjectTypeManagerButtonDefaultAddNew() {
      const valueAuthority = await NCoreHelper.v2checkAuthorities('UserTypeManager')
      if ((valueAuthority & NCoreConfig.authority_Add) !== 0) {
        await this.callStoreCrudUpdate({
          ...
          title: "Thêm mới kiểu người dùng",
          type: "add",
          funcName: "kiểu người dùng",
          groupTable: 2,
          data: null
        });
    ...
    async subEvSubjectTypeManagerAction(val) {
      switch (val.id) {
        ...
        case 101: {
          ...
          await this.callStoreCrudUpdate({
            id: this.$refs.refSubjectTypeManagerEvent.idModal,
            title: "Cập nhật kiểu người dùng",
            type: "update",
            funcName: "kiểu người dùng",
            groupTable: 2,
