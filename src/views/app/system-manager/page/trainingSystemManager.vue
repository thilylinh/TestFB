<template>
  <b-row>
    <b-colxx class="disable-text-selection">
      <b-row>
        <b-colxx xxs="12">
          <h1 class="NHeaderPage">Quản lý Hệ đào tạo</h1>
...
export default {
  ...
  data() {
    return {
      pageId: "trainingSystemManagerPageId",
  ...
  methods: {
    ...
    async fetchData() {
      ...
      const iResult = await NCoreHelper.v2executeGET(
        this,
        trainingSystemManagerApi.GET_PAGING,
        dataSend
      );
    ...
    async subEvSubjectTypeManagerButtonDefaultAddNew() {
      const valueAuthority = await NCoreHelper.v2checkAuthorities('TrainingSystemManager')
    ...
    async subEvSubjectTypeManagerDelete(val) {
      ...
      const iResult = await NCoreHelper.v2executePOST(
        this,
        trainingSystemManagerApi.DELETE,
        {
          models: model
        }
      );
