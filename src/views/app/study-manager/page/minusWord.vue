<template>
  <b-row>
    <b-colxx class="disable-text-selection">
      <b-row>
        <b-colxx xxs="12">
          <h1 class="NHeaderPage">Quản lý từ khóa loại trừ</h1>
          ...
          <div class="mb-2">
            ...
            <b-collapse id="displayOptions" class="d-md-block">
              <div class="d-block d-md-inline-block pt-1">
                ...
                <!--                </div>-->
                <n-button-default-search ref="refSubjectTypeManager_ButtonDefaultSearch"
                                         @handlerSubmitEvent="subEvSubjectTypeManager_ButtonDefaultSearch"/>
          ...
          <b-colxx xxs="12" class="NTable">
            <vuetable ref="refSubjectTypeManagerTable"
              ...
              <template slot="roomName" slot-scope="props">
                <span class="NTitleDataInTable">{{ props.rowData.root }} / {{ props.rowData.replace }}</span>
    ...
    <minus-word-event ref="refSubjectTypeManagerEvent"
...
import MinusWordEvent from "@/views/app/study-manager/event/minusWordEvent.vue";
...
export default {
  ...
  methods: {
    ...
    async subEvSubjectTypeManagerButtonDefaultAddNew() {
      const valueAuthority = await NCoreHelper.v2checkAuthorities('MinusWord')
      if ((valueAuthority & NCoreConfig.authority_Add) !== 0) {
        await this.callStoreCrudUpdate({
          ...
          title: "Thêm mới từ khóa thay thế",
          type: "add",
          funcName: "từ khóa thay thế",
          groupTable: 1,
          data: null
        });
    ...
    async subEvSubjectTypeManager_ButtonDefaultSearch(val) {
      this.search = val;
      this.pageNumber = 1;
      await this.fetchData();
    },
    async subEvSubjectTypeManagerAction(val) {
      switch (val.id) {
        ...
        case 101: {
          ...
          await this.callStoreCrudUpdate({
            id: this.$refs.refSubjectTypeManagerEvent.idModal,
            title: "Cập nhật từ khóa thay thế",
            type: "update",
            funcName: "từ khóa thay thế",
            groupTable: 1,
            data: await NCoreHelper.v2executeGETById(
              this,
              minusWordApi.GET_BY_ID,
              val.dataContentId
            )
          });
          NCoreHelper.v2modalShowHide(
